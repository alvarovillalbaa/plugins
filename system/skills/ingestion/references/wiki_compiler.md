...
Load the parent brain contract before writes: `../brain/references/brain_contract.md`. That contract decides the `BRAIN.md` boundary, AFS/native adaptation mode, raw retention rule, and canonical knowledge path.
...
Input scope is normally both raw source material and repo-local Memory. Memory entries are evidence of experience; promote only durable learnings, facts, procedures, decisions, or recurring patterns into canonical pages.
...
Do not promote unreadable, metadata-only, or blocked sources into factual claims. Keep them in the raw queue or blocked ledger until primary content is readable.
