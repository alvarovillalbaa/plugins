...
- Tests live in `tests/` only: `unit/`, `integration/`, `e2e/`, `smoke/`, `scripts/`.
...
Choose the lowest that still proves the claim:
- **No DB**: pure logic only
- **Test DB** (default): managed by pytest-django
- **Real DB**: when behavior depends on indexes, constraints, or query plans — use local containers or ECS via `tests/scripts/run_ecs_tests.sh`
