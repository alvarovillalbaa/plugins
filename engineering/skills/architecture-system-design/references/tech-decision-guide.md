...
**Write-Through:**
```
Write: Write to cache → cache writes to DB → return success
Read:  Read from cache (always hit)
```
...
**RabbitMQ:** Complex routing (topic, fanout, headers), request/reply, priority queues, message acknowledgment critical.

**Kafka:** Event sourcing, high throughput (>50K msg/sec), message replay needed, stream processing, log aggregation.

**SQS:** AWS-native apps, simple queue semantics, serverless, no infra management.

**Redis Streams:** Already using Redis, moderate throughput, simple streaming, real-time features.
...
**Go multi-cloud when:** Regulatory requirements mandate it; a specific service (e.g., GCP BigQuery) is best-in-class; need negotiating leverage.

**Stay single-cloud when:** Small team; want to minimize complexity; need deep integration.
