...
Truncation-based cleanup, as below, is **Tier 3 only** — a local replica database you are free to
destroy. At Tier 4 (staging replica) use a per-test transaction that always rolls back, because the
database is shared and truncation destroys other people's data. At Tier 5 no cleanup exists, because
no writes exist. See
