...
class BreakingChangeDetector:
    ...
    def _compare_object_properties(self, base_path: str, old_schema: Dict, new_schema: Dict, context: str) -> None:
        ...
        for prop_name in added_props:
            if prop_name not in new_required:
                self.report.add_change(Change(
                    change_type=ChangeType.NON_BREAKING,
                    severity=ChangeSeverity.INFO,
                    category="schema",
    ...
    def _generate_endpoint_removal_migration(self, removed_path: str, method: str,
        ...
        for existing_path in remaining_paths.keys():
            ...
            if len(existing_segments) == len(path_segments):
                ...
                if similarity >= len(path_segments) * 0.5:
    ...
    def generate_text_report(self) -> str:
        ...
        if not self.report.changes:
            lines.extend(["No changes detected between the API versions!", ""])
        else:
            ...
            if breaking_changes:
                lines.extend(["BREAKING CHANGES:", "=" * 60])
            ...
            if potentially_breaking:
                lines.extend(["POTENTIALLY BREAKING CHANGES:", "=" * 60])
                for change in sorted(potentially_breaking, key=lambda x: x.severity.value):
                    self._add_change_to_report(lines, change)
                lines.append("")

            if non_breaking:
                lines.extend(["NON-BREAKING CHANGES:", "=" * 60])
                for change in non_breaking:
                    self._add_change_to_report(lines, change)
                lines.append("")

            if enhancements:
                lines.extend(["ENHANCEMENTS:", "=" * 60])
        ...
        if self.report.has_breaking_changes():
            ...
            if breaking_count > 0:
                lines.extend([
                    ...
                    "   definitely break existing clients. A major version bump is required.",
                    ""
                ])
                ...
                lines.extend([
                    ...
                    "   changes. Consider a minor version bump and communicate changes to clients.",
                    ""
                ])
        else:
            lines.extend([
    ...
    def _add_change_to_report(self, lines: List[str], change: Change) -> None:
        ...
        lines.extend([
            f"[{change.severity.value.upper()}] {change.message}",
            f"   Path: {change.path}",
...
Examples:
  ...
  python breaking_change_detector.py --output report.txt v1.json v2.json
    ...
    parser.add_argument('old_spec', help='Old API specification file (JSON format)')
    parser.add_argument('new_spec', help='New API specification file (JSON format)')
