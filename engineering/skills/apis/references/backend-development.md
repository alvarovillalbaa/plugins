...
- default to hard cuts: no backfills, compatibility aliases, or expand-contract choreography unless the user explicitly chooses a temporary production-migration path
- update the canonical schema owner and all active callers together when the repo can safely do so
- stop and ask before preserving old shapes solely because old callers might exist
- check downstream contracts such as API schemas, workers, analytics payloads, caches, and generated clients
- update focused tests and the nearest contract docs for any public shape change
...
- Validate once at the boundary, then operate on canonical data. Do not normalize alternate client payload shapes; reject them and fix the producer.
