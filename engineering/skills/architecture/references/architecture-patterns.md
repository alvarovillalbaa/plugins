...
const orderCreated: DomainEvent = {
  ...
  timestamp: new Date(),
  payload: { customerId: "cust-789", items: [...], total: 99.99 },
  metadata: { correlationId: "req-001", causationId: "cmd-create-order" }
};
```
...
// Read side (Queries) — queries against denormalized read model
class OrderQueryHandler {
  async handle(query: OrderSummaryQuery): Promise<OrderSummary[]> {
    return this.readDb.query(
      `SELECT * FROM order_summaries WHERE customer_id = ? AND created_at BETWEEN ? AND ?`,
...
class Order {
  ...
  private apply(event: OrderEvent): void {
    switch (event.type) {
      case 'OrderCreated': this.state = { status: 'created', items: event.items }; break;
      case 'ItemAdded': this.state.items.push({ id: event.itemId, qty: event.quantity }); break;
      case 'OrderShipped': this.state.status = 'shipped'; this.state.trackingNumber = event.trackingNumber; break;
    }
  }
}
```
...
## Team Size → Default Pattern

| Team Size | Recommended Starting Point |
|-----------|---------------------------|
| 1-3 developers | Modular Monolith |
| 4-10 developers | Modular Monolith or service-oriented |
| 10-15 developers | Service-oriented or consider microservices |
| 15+ developers | Microservices (when domain boundaries are clear) |
