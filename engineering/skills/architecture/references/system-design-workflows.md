...
For each major component, address:
...
With headroom (70% target): 180 / 0.7 = 257 cores = 32 servers × 8 cores
```
...
**Capacity plan document should include:**
...
**Vertical scaling (scale up):** More CPU/memory/disk. Simpler but has limits. Use when single server can handle more.

**Horizontal scaling (scale out):** More servers. Requires stateless design. Use when linear scaling is needed.
...
**Staged routing exception (use only with explicit approval):**
```
1. Define the canonical owner boundary and target behavior.
2. Switch active callers to the new owner with focused proof.
3. Use staged routing only for live production safety, not as a compatibility facade.
4. Remove the old path in the same approved migration window.
```
